import React from 'react';
import fetch from 'isomorphic-unfetch';

// Use a global to save the user, so we don't have to fetch it again after page navigations
let userState;

const User = React.createContext({ user: null, loading: false });

export const fetchUser = async () => {
  if (userState !== undefined) {
    return userState;
  }

  const storedUser = JSON.parse(sessionStorage.getItem('user'));
  const shouldRefresh = JSON.parse(sessionStorage.getItem('shouldRefresh'));

  if (storedUser && !shouldRefresh) {
    return storedUser;
  }

  if (shouldRefresh) {
    sessionStorage.removeItem('user');
  }

  let userData;
  try {
    userData = await fetch(`/api/auth0/me`);
    userState = userData.ok ? await userData.json() : null;
  } catch (error) {
    console.error(error);
  }

  sessionStorage.setItem('user', JSON.stringify(userState));
  sessionStorage.setItem('shouldRefresh', false);

  return userState;
};

export const UserProvider = ({ value, children }) => {
  const { user } = value;

  // If the user was fetched in SSR add it to userState so we don't fetch it again
  React.useEffect(() => {
    if (!userState && user) {
      userState = user;
    }
  }, []);

  return <User.Provider value={value}>{children}</User.Provider>;
};

export const useUser = () => React.useContext(User);

export const useFetchUser = () => {
  const [data, setUser] = React.useState({
    user: userState || null,
    loading: userState === undefined,
  });

  React.useEffect(() => {
    if (userState !== undefined) {
      return;
    }

    let isMounted = true;

    fetchUser().then((user) => {
      // Only set the user if the component is still mounted
      if (isMounted) {
        setUser({ user, loading: false });
      }
    });

    return () => {
      isMounted = false;
    };
  }, [userState]);

  return data;
};
